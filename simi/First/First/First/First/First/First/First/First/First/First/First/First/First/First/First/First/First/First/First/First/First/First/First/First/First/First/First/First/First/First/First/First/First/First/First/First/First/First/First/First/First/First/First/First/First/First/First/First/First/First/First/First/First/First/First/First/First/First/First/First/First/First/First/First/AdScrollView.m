...
@synthesize delegateGHP = _delegateGHP;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.contentSize = CGSizeMake(UISCREENWIDTH * 3, 244/2);
        self.delegate = self;
...
#pragma mark - 设置每个图片跳转的URL
- (void)setImgGoUrlArray:(NSMutableArray *)imgGoUrlArray
{
    _imgGoUrlArray = imgGoUrlArray;
    imgToUrl = [NSString stringWithFormat:@"%@",_imgGoUrlArray[1]];
}
#pragma mark - 设置每个对应广告对应按钮
- (void)createBtn{
    ...
    _leftBtn.frame = FRAME(0, 0, UISCREENWIDTH, UISCREENHEIGHT);
    _leftBtn.backgroundColor = [UIColor clearColor];
    _leftBtn.tag = 10;
    [_leftBtn addTarget:self action:@selector(btntouch:) forControlEvents:UIControlEventTouchUpInside];
    [_leftImageView addSubview:_leftBtn];

    _centerBtn = [[UIButton alloc]init];
    _centerBtn.frame = FRAME(0, 0, UISCREENWIDTH, UISCREENHEIGHT);
    _centerBtn.backgroundColor = [UIColor clearColor];
    _centerBtn.tag = 20;
    [_centerBtn addTarget:self action:@selector(btntouch:) forControlEvents:UIControlEventTouchUpInside];
    [_centerImageView addSubview:_centerBtn];
    
    _rightBtn = [[UIButton alloc]init];
    _rightBtn.frame = FRAME(0, 0, UISCREENWIDTH, UISCREENHEIGHT);
    _rightBtn.backgroundColor = [UIColor clearColor];
    _rightBtn.tag = 30;
    [_rightBtn addTarget:self action:@selector(btntouch:) forControlEvents:UIControlEventTouchUpInside];
    [_rightImageView addSubview:_rightBtn];
}
- (void)btntouch:(UIButton *)btn{
    
    NSLog(@"%@",imgToUrl);
    [self.delegateGHP goUrlIs:imgToUrl];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    {
        currentImage = (currentImage+_imageNameArray.count-1)%_imageNameArray.count;
        _pageControl.currentPage = (_pageControl.currentPage +_imageNameArray.count-1)%_imageNameArray.count;
    }
