...
//对应农历的年月日  我写的哦！！！！
+(NSString *)GetLunarDateTimeForYear:(int)year month:(int)month date:(int)date
{
    JBCalendar* date1 = [[JBCalendar alloc]init];
    date1.year = year,date1.month =month,date1.day = date;
    LunarCalendar *lunarCalendar = [[date1 nsDate] chineseCalendarDate];
    NSString * lunar = [[NSString alloc]initWithFormat:
                        @"%@%@年%@%@",lunarCalendar.YearHeavenlyStem,lunarCalendar.YearEarthlyBranch,lunarCalendar.MonthLunar,lunarCalendar.DayLunar];
    return lunar;
}
